...
class CacheList:
    ...
    def update(self, cid, content):
        # YOUR CODE STARTS HERE
        if cid in self:
            difference = content.size - self.head.value.size
            if self.remainingSpace < difference:
                return "Update not allowed due to size limit"
            self.head.value = content
            self.remainingSpace -= difference
